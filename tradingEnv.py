...
class TradingEnv(gym.Env):
    ...
    def __init__(self, marketSymbol, startingDate, endingDate, money, stateLength=30,
        ...
        # CASE 2: Real stock loading
            ...
            # Otherwise, download the stock market data from Yahoo Finance and save it in the database
                ...
                print('data is not yet imported')
    ...
    def step(self, action):
        ...
        # local variables for differential downside deviation ratio by moodySaffel
        A = 0.00001 # to avoid division by 0
        nu = 0.001
        DD2 = 0.00001 # to avoid division by 0
        DDR = 0 #https://realworld-sdm.github.io/paper/23.pdf
        dddr = 0.001

        # local variables for sharpe ratio
        riskFreeRate_reward = 0.01
        ...
        if not customReward:
            ...
            #### standard reward from original paper
            ...
            """
            #### DDR reward
            if self.data['Returns'][t] > 0:
                dddr = (self.data['Returns'][t] - 0.5 * A)\
                              /(math.sqrt(DD2))
            else:
                dddr = (DD2*(self.data['Returns'][t]-0.5 * A) - 0.5*A*self.data['Returns'][t]**2)\
                              /(DD2 * math.sqrt(DD2))

            A = A + nu*(self.data['Returns'][t] - A)
            DD2 = DD2 + nu*(min(self.data['Returns'][t],0)**2-DD2)
            DDR = DDR + nu*dddr
            self.reward = DDR
            #### DDR reward end
            """
            ...
            expectedReturn_reward = self.data['Returns'].mean()

            # Compute the returns volatility
            volatility_reward = self.data['Returns'].std()

            # Compute the Sharpe Ratio (252 trading days in 1 year)
            if expectedReturn_reward != 0 and volatility_reward != 0:
                self.reward = np.sqrt(252) * (expectedReturn_reward - riskFreeRate_reward) / volatility_reward
            else:
                self.reward = 0
            #### sharpe ratio reward end
            """
        else:
            self.reward = (self.data['Close'][t-1] - self.data['Close'][t])/self.data['Close'][t-1]
            print('we have a different reward')
